...
/** Открывает чат в Telegram по уже известному номеру компании — без юзернейма. */
const COMPANY_TELEGRAM_URL = 'https://t.me/+79859330121'
const LEAD_COMMENT = 'Заявка со страницы /telegram — хочет перейти в Telegram-чат с менеджером.'
...
export default function TelegramLeadForm() {
  ...
  useDeferredLead(phone, 'telegram', { comment: LEAD_COMMENT })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setPhoneError(false)
    setStatus('loading')
    const ok = await sendLead({ phone, comment: LEAD_COMMENT, source: 'telegram', _honey: honey })
    if (ok) {
      setStatus('ok')
      window.location.href = COMPANY_TELEGRAM_URL
    } else {
      setStatus('error')
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        {status === 'loading' ? 'Отправка...' : 'Перейти в Telegram'}
      </button>
